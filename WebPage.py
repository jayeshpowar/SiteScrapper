...
class WebPage:
    ...
    def add_errors(self, errors):
        """
        Utility method to add js errors fetched from the page. Note that this is
         useful only if the logging has been added to the selenium firefox
         driver.
        """
        for error_entry in errors:
            if "Error" in error_entry["message"] and "SEVERE" in error_entry[
                'level']:
            ...
            if not self.external_url:
                if 'text/html' in content_type:
                    ...
                    for a_link in link_elements:
                        ...
                        if link is not None:
                            link_page = WebPage(link, self.base_domain,
                                                self.base_site, self.driver)
                ...
                print("Visited ", self.url, " and added ", link_count,
                      " links to visit ")

    def fetch_links_for_page(self):
        """
    ...
    def fetch_url(self):
        ...
        if self.is_valid_web_page():
            try:
                ...
                fetched_page = self.fetch_links_for_page()
